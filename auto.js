      ...
      try {
        var listenEmitter = api.listenMqtt(async (error, event) => {
          ...
          if (hasPrefix && aliases(command)?.hasPrefix === false) {
            api.sendMessage(`⚠️Invalid usage this command doesn't need a prefix`, event.threadID, event.messageID);
            return;
          }
          ...
          if (event.body && command && prefix && event.body?.toLowerCase().startsWith(prefix.toLowerCase()) && !aliases(command)?.name) {
            api.sendMessage(`⚠️Invalid command '${command}' please use ${prefix}help to see the list of available commands.`, event.threadID, event.messageID);
            return;
          }
          for (const {
              handleEvent,
